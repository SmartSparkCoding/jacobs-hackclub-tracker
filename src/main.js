    ...
    <section class="content-grid">
      <section class="panel form-panel">
        <div class="panel__header">
          ...
          <span class="status-pill" id="connectionStatus">Loading Firebase…</span>
        </div>
...
const renderEventTypes = () => {
  ...
  elements.eventTypeList.innerHTML = state.eventTypes
    .map(
      (eventType) => `
        <span class="type-chip">${eventType}</span>
      `,
    )
    .join('');
};
...
const readDemoState = (key, fallback) => {
  try {
...
const writeDemoState = (key, value) => {
  window.localStorage.setItem(key, JSON.stringify(value));
};

const syncLocalState = () => {
  state.entries = readDemoState(STORAGE_KEYS.demoEntries, []);
  state.eventTypes = readDemoState(STORAGE_KEYS.demoEventTypes, DEFAULT_EVENT_TYPES);
  renderEventTypes();
  renderEntries();
...
const addLocalEntry = (entry) => {
  ...
  writeDemoState(STORAGE_KEYS.demoEntries, state.entries);
  renderEntries();
};
...
const addLocalType = (typeName) => {
  ...
  writeDemoState(STORAGE_KEYS.demoEventTypes, state.eventTypes);
  renderEventTypes();
};
...
const ensureDefaultTypes = async () => {
  ...
  const existingTypes = await getDocs(collection(firestore, 'eventTypes'));
  if (!existingTypes.empty) {
    return;
  }

  for (const eventType of DEFAULT_EVENT_TYPES) {
    await addDoc(collection(firestore, 'eventTypes'), {
      name: eventType,
      createdAt: Date.now(),
    });
  }
...
const bootstrap = async () => {
  ...
  try {
    ...
    const entriesQuery = query(collection(firestore, 'entries'), orderBy('createdAt', 'desc'));
    const eventTypesQuery = query(collection(firestore, 'eventTypes'), orderBy('createdAt', 'asc'));
    ...
    unsubEventTypes = onSnapshot(eventTypesQuery, (snapshot) => {
    ...
    console.error(error);
    syncLocalState();
    setStatus('Firebase config error', 'danger');
  }
};
...
elements.eventTypeSelect.addEventListener('change', () => {
  ...
  if (firebaseReady) {
    addDoc(collection(firestore, 'eventTypes'), {
      name,
      createdAt: Date.now(),
    }).catch((error) => {
      console.error(error);
